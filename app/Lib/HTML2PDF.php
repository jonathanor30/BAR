...
if (!defined('__CLASS_HTML2PDF__')) {
    ...
    $road = explode(SEPARATOR . 'app', $route);
    $test = $road[0];
    
    //Ruta librerias pdf
    $pdfRoute = $test.SEPARATOR.'public'.SEPARATOR.'Pdf'.SEPARATOR.'_class'.SEPARATOR;
     //Puedes probar la ruta de las librearias 
     //echo $pdfRoute;
    
    require_once $pdfRoute . 'exception.class.php';
    require_once $pdfRoute . 'locale.class.php';
    require_once $pdfRoute . 'myPdf.class.php';
    require_once $pdfRoute . 'parsingHtml.class.php';
    require_once $pdfRoute . 'parsingCss.class.php';
...
' . $content . '
        ...
        protected function _calculateTableCellSize(&$cases, &$corr)
        {
            ...
            //para cada celda sin colspan, obtenemos el ancho máximo para cada columna
            $sw = array();
            ...
            //Para cada celda con colspan, adaptamos el ancho de cada columna.
            for ($x = 0; $x < count($corr[0]); $x++) {
                for ($y = 0; $y < count($corr); $y++) {
                    if (isset($corr[$y][$x]) && is_array($corr[$y][$x]) && $corr[$y][$x][2] > 1) {

                        //Suma el ancho máximo de cada columna en colspan.
                        $s = 0;for ($i = 0; $i < $corr[$y][$x][2]; $i++) {
                            $s += $sw[$x + $i];
                        }

                        //si el ancho máximo es <el ancho de la celda con colspan => adaptamos el ancho de cada ancho máximo
